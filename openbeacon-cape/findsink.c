...
print_hardware_address_ip(const char *if_nam, unsigned char hw_address[]) {
	...
	/*display mac address*/
	printf("%s  -  %.2x:%.2x:%.2x:%.2x:%.2x:%.2x  -  " , if_nam, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
...
int checkdatasink(const char* interface, const char *iface);
...
int checkdatasink(const char *interface, const char *iface)
{
	...
	ipbytes[1] = (ip_addr >> 8) & 0xFF;
	ipbytes[2] = (ip_addr >> 16) & 0xFF;
	ipbytes[3] = (ip_addr >> 24) & 0xFF;
	...
	serverAddr.sin_addr.s_addr = inet_addr(iface);/*"127.0.0.1");*/
	...
	if (setsockopt (clientSocket, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv)) < 0)
	{
		close(clientSocket);
		return 0;
	}
...
*/
	...
	if (send(clientSocket, "c67e7c76-1104-4afd-ab97-2b714a1bfd6a", 36, 0) != 36)
	{
		close(clientSocket);
		return 0;
	}
	...
	if (send(clientSocket, source_mac, 6, 0) != 6)
	{
		close(clientSocket);
		return 0;
	}
	if (send(clientSocket, ipbytes, 4, 0) != 4)
	{
		close(clientSocket);
		return 0;
	}
	tv.tv_sec = 5;  /* 2 Secs Timeout*/
	tv.tv_usec = 0;  /* Not init'ing this can cause strange errors*/
	...
	if (recv(clientSocket, buffer, 36, 0/*MSG_PEEK*/) != 36)
	{
		close(clientSocket);
		if (!quiet_flag)
	...
	if (strcmp(buffer, "7784c4d9-b39f-4f7b-81c9-8b1fc30fa342"))
	{
		close(clientSocket);
		if (!quiet_flag)
	...
	/*if (!quiet_flag)*/
		printf("L%d:Data Sink found on %s!\n",__LINE__, iface);
...
/*
	...
	{
		if (checkdatasink(if_name, host)==1)
		{
			cleanup();
			return 1;
		}
...
*/
	...
	{
		if ( checkdatasink(if_name, it->str) == 1 )
		{
            memset(host, '\0', 16);
			strcpy(host,it->str);
            //printf("\n\n%d -> %d, \t %s \t %s\n\n", strlen(it->str), strlen(host), it->str, host);
			//printf("\nL%d: Why?", __LINE__);
			cleanup();
			//printf("\nL%d: Why?", __LINE__);
			return 1;
